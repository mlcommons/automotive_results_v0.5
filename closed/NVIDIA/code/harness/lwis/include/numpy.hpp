...
public:
        ...
        , m_FStream(m_Path)
    {
        ...
        m_FStream.read((char*) m_Header.data(), m_HeaderSize);
        ...
        // parse header
        std::regex re(R"re(\{'descr': '[<|][fib]([\d])', 'fortran_order': False, 'shape': \(([\d, ]*|)\), \} +\n)re");
        std::smatch matches;
        ...
        else
        {
            ...
            std::transform(
                dims.begin(), dims.end(), m_TensorDims.begin(), [](const std::string& s) { return std::stol(s); });
        }
        // check header sanity
        size_t tensorSize
    ...
    // stream raw bytes without loading full file to memory
    void streamBytes(std::vector<char>& dst, const size_t offset, const size_t numBytes)
    {
        const size_t file_offset = 10 + m_HeaderSize + offset;
        m_FStream.seekg(file_offset, std::ios::beg);
        dst.resize(numBytes);
        m_FStream.read(dst.data(), numBytes);
    }

    // stream samples without loading full file to memory
    void streamSamples(std::vector<char>& dst, const std::vector<size_t>& indices)
    {
        const size_t baseOffset = 10 + m_HeaderSize;
        const size_t sampleBytes
            = std::accumulate(m_TensorDims.begin() + 1, m_TensorDims.end(), m_ElementSize, std::multiplies<size_t>());
        dst.resize(sampleBytes * indices.size());

        for (size_t i = 0; i < indices.size(); i += 1)
        {
            m_FStream.seekg(baseOffset + indices[i] * sampleBytes, std::ios::beg);
            m_FStream.read(dst.data() + i * sampleBytes, sampleBytes);
        }
    }
