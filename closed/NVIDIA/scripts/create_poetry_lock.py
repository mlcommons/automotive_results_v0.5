...
if __name__ == "__main__":
    ...
    subprocess.run("python3 -m pip install poetry", shell=True)
    ...
    for path in paths:
        ...
        for package in packages:
            ...
            poetry_cmd = f"python3 -m poetry add '{package_name}'"
            if package_name in url_mapping:
                poetry_cmd = f"python -m poetry add '{url_mapping[package_name]}'"
    ...
    name = '"MLPerf-Automotive"'
    author = '"MLPerf-Automotive"'
    py_version = '"^3.12"'
    poetry_init_cmd = f'python3 -m poetry init --no-interaction --name {name} --author {author} --python {py_version}'
