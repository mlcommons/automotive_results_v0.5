...
def allocate_buffers(engine: trt.ICudaEngine,
    ...
    Returns:
        ...
        outputs (List[MemBuffer] | List[HostDeviceBuffer]): output memory address pair
    ...
    for tensor_idx in range(num_io_tensors):
        ...
        if soc_physical_memory:
            _buff = MemBuffer(tensor_shape,
                              nptype(tensor_dtype),
                              name=tensor_name,
                              )
        else:
...
class EngineRunner:
    ...
    def __init__(self,
        ...
        self.input_tensor_map, self.outputs = allocate_buffers(self.engine, profile_id, soc_physical_memory)
        self.stream = cudart.cudaStreamCreate()
        self.context = self.engine.create_execution_context()
        self.soc_physical_memory = soc_physical_memory


        # Set context tensor address (equivalent to set binding in old syntax)
        if self.soc_physical_memory:
            for i in self.input_tensor_map.values():
                self.context.set_tensor_address(i.name, int(i.buffer))
            for o in self.outputs:
                self.context.set_tensor_address(o.name, int(o.buffer))
        else:
            for i in self.input_tensor_map.values():
    ...
    def __call__(self,
        ...
        if self.soc_physical_memory:
            [buff.h2d_prefetch(self.stream, input=input) for (buff, input) in zip(input_buffers, inputs)]
        else:
            [buff.h2d_async(self.stream, host=input) for (buff, input) in zip(input_buffers, inputs)]
        ...
        if self.soc_physical_memory:
            # prefetch output data for host access
            [out.d2h_prefetch(self.stream) for out in self.outputs]
        else:
            # copy outputs to host
            [out.d2h_async(self.stream) for out in self.outputs]
        ...
        if self.soc_physical_memory:
            return [out.buffer_to_ndarray(copy=copy_outputs,
