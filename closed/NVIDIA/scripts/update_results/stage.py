...
def copy_mlpauto_run(src_dir: str, dst_dir: str, file_list: List[str], dry_run: bool = False):
...
def copy_performance_run(src_dir: str, output_dir: str, run_num: int, has_power: bool = False, dry_run: bool = False):
    ...
    if has_power:
        ...
        copy_mlpauto_run(src_dir,
                        ...
                        dry_run=dry_run)
        # Copy ranging runs
        copy_mlpauto_run(src_dir.replace(f"run_{run_num}", "ranging"),
        ...
        copy_mlpauto_run(os.path.join(src_dir[:src_dir.find(f"run_{run_num}") - 1], "power"),
...
def stage_result_candidates(candidates: Tree, staging_dir: str, dry_run: bool = False):
        ...
        for benchmark, scenarios in benchmarks.items():
            ...
            for scenario, runs in scenarios.items():
                ...
                if "PerformanceOnly" in runs and "AccuracyOnly" in runs:
                    ...
                    copy_mlpauto_run(runs["AccuracyOnly"][0],
                                    output_dir,
                                    ["mlperf_log_accuracy.json",
                                     "mlperf_log_detail.txt",
                                     "mlperf_log_summary.txt",
                ...
                for audit_test in AuditTest:
                    ...
                    safe_copytree(os.path.join(runs[audit_name][0], audit_name),
                                  ...
                                  dry_run=dry_run)
                    # Copy the metadata
                    copy_mlpauto_run(runs[audit_name][0],
                                    output_dir,
                                    ["metadata.json"],
                                    dry_run=dry_run)
    ...
    for system_name, benchmarks in candidates.tree.items():
        logger.inc_indent_level()
        for benchmark, scenarios in benchmarks.items():
            # FIXME: enable this if we achieve 99.9% accuracy with 99% accuracy model, i.e. both are from the same run
            if benchmark in {"ssd-resnet50", "deeplabv3plus"} and False:
