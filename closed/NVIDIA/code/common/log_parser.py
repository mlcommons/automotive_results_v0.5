...
scenario_loadgen_log_keys: Final[Dict[str, str]] = {
    Scenario.SingleStream: "result_99.90_percentile_latency_ns",
    Scenario.ConstantStream: "result_99.90_percentile_latency_ns",
}
...
def perf_metric_from_scenario(system_id: str, scenario: str, benchmark: str = None):
    ...
    if "MaxQ" in system_id:
        if scenario in [Scenario.SingleStream, Scenario.ConstantStream]:
...
def extract_mlperf_result_from_log(log_dir: str, system_id: str, benchmark: str,
    ...
    The MaxP Perf number should be:
        - SingleStream: 99.9-percentile per-query latency
        - ConstantStream: 99.9-percentile per-query latency
    The MaxQ perf number is:
        - SingleStream/ConstantStream: joules per stream (calculated by average_power * mean_latency)
    ...
    if "MaxQ" not in system_id:
        ...
        # For MaxQ, singlestream and constantstream uses "joules_per_stream"
...
def get_perf_regression_ratio(new_result: float, results_dir: str, system_id: str, benchmark: str, scenario: str):
    ...
    # For MaxP, we use percentile latency comparison.
    if scenario in [Scenario.SingleStream, Scenario.ConstantStream]:
