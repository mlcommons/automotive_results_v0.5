...
@dataclass
class MemBuffer:
    """Represents a unified Host and Device memory buffer, accessible coherently by both host and device."""
    shape: Tuple[int, ...]
    ...
    def __post_init__(self):
        ...
        if self.buffer is None:
            self.buffer = CUDARTWrapper.cudaMallocManaged(self._bytesize, CUDARTWrapper.cudaMemAttachGlobal)
            logging.debug(f"Allocated buffer {self.buffer} for {self.name}")

    def __del__(self):
        if self.buffer is not None:
            logging.debug(f"Freeing buffer {self.buffer} for {self.name}")
            CUDARTWrapper.cudaFree(self.buffer)

    def _check_buffer(fn: Callable):
        def _f(inst, *args, **kwargs):
            if inst.buffer is None or inst.buffer == 0:
                raise BufferError(f"Buffer for {inst.name} is not allocated.")

            return fn(inst, *args, **kwargs)
        return _f

    @_check_buffer
    def h2d_prefetch(self,
                     stream: cudart.cudaStream_t,
                     input: Optional[np.ndarray] = None):
        if self.buffer is None or self.buffer == 0:
            raise BufferError(f"Buffer for {self.name} is not allocated.")
        if input is not None:
            CUDARTWrapper.cudaMemcpyAsync(self.buffer,
                                          input.tobytes(),
                                          self._bytesize,
                                          CUDARTWrapper.cudaMemcpyKind.cudaMemcpyHostToHost,
                                          stream)
        CUDARTWrapper.cudaStreamAttachMemAsync(stream, 
                                               self.buffer, 
                                               self._bytesize, 
                                               CUDARTWrapper.cudaMemAttachGlobal)


    @_check_buffer
    def d2h_prefetch(self,
                     stream: cudart.cudaStream_t):
        if self.buffer is None or self.buffer == 0:
            raise BufferError(f"Buffer for {self.name} is not allocated.")
        CUDARTWrapper.cudaStreamAttachMemAsync(stream, 
                                               self.buffer, 
                                               self._bytesize, 
                                               CUDARTWrapper.cudaMemAttachHost)

    @_check_buffer
    def buffer_to_ndarray(self,
                          copy: bool = True,
                          read_only: bool = False) -> np.ndarray:
        return np_from_pointer(self.buffer,
                               self.shape,
                               self.nptype,
                               copy=copy,
                               read_only=read_only)
