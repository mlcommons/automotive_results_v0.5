...
class RunHarnessHandler(GenerateConfFilesHandler):
    ...
    def __init__(self, benchmark_conf, power_controller, profiler=None, measure_power=False, use_py_harness=False, skip_postprocess=False):
        ...
        Args:
            ...
            skip_postprocess (bool): If True, skips accuracy log parsing and statistics reporting in cleanup().
                                     (Default: False)
        """
        super().__init__(benchmark_conf, skip_file_checks=False)
        self.action = Action.RunHarness

        assert measure_power == False, "Measure power is not supported for MLPerf Automotive yet"
        ...
        if self.profiler is not None:
            logging.warn("CUDA graphs results in a CUDA illegal memory access when run with a profiler. Force-disabling CUDA graphs.")
    ...
    def _get_result_string(self, result_data):
        if result_data["test_mode"] == "AccuracyOnly":
            ...
            min_duration_met = result_data.get("effective_min_duration_ms", 0) >= 600000 # FIXME: jsuh - do we need 10min min runtime for mlperf automotive?
