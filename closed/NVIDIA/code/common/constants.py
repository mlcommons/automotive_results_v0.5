...
from code.common import __MLPERF_AUTO_VERSION__
...
# example certain scripts in CI/CD. Provide an environment variable 'OUTSIDE_MLPAUTO_ENV' to allow using constants.py
# outside of the docker.
if importlib.util.find_spec("tensorrt") is not None and os.environ.get("OUTSIDE_MLPAUTO_ENV", "0") == "0":
...
VERSION: Final[str] = __MLPERF_AUTO_VERSION__
...
"""str: Path to MLPerf Automotive build directory. Inherited from environment."""

G_MLCOMMONS_REPO_PATH: Final[str] = G_BUILD_DIR.joinpath("automotive")
"""str: Path to the MLCommons automotive repo."""

G_RESULTS_SUBMISSION_PATH: Final[str] = os.environ.get("ARTIFACTS_DIR", G_BUILD_DIR.joinpath("artifacts"))
"""str: Path to the MLPerf Automotive results to submit"""
...
class Benchmark(AliasedNameEnum):
    ...
    BEVFormer: AliasedName = AliasedName("bevformer", ("bevformer-tiny", "bevformer-99", "bevformer-tiny-99",))

    @classmethod
    def PYTHON_PATH(cls, benchmark):
        return "/usr/bin/python3"
@unique
class Scenario(AliasedNameEnum):
    """Names of supported workload scenarios in MLPerf Automotive."""

    SingleStream: AliasedName = AliasedName("SingleStream", ("single-stream", "single_stream", "singlestream"))
    ConstantStream: AliasedName = AliasedName("ConstantStream", ("constant-stream", "constant_stream", "constantstream"))


@unique
class Action(AliasedNameEnum):
    """Names of actions performed by our MLPerf Automotive pipeline."""
...
class HarnessType(AliasedNameEnum):
    ...
    Python: AliasedName = AliasedName("python")
...
class AccuracyTarget(Enum):
    ...
    def toString(self):
        if self == AccuracyTarget.k_99:
            return "99%"
        elif self == AccuracyTarget.k_99_9:
            return "99.9%"
        elif self == AccuracyTarget.k_99_99:
            return "99.99%"
        else:
            return "Unknown"
    def fromString(self, str):
        if str == "99%":
            return AccuracyTarget.k_99
        elif str == "99.9%":
            return AccuracyTarget.k_99_9
        elif str == "99.99%":
            return AccuracyTarget.k_99_99
...
class WorkloadSetting:
    ...
    MLPerf Automotive allows (i.e. power submission), or running the same workload with different software.
    """
...
"""Tuple[Benchmark, ...]: Benchmarks for the """


G_AUTOMOTIVE_BENCHMARKS: Tuple[Benchmark, ...] = (
    Benchmark.BEVFormer,
)
"""Tuple[Benchmark, ...]: Benchmarks for Automotive, running in the vehicle standalone"""


G_DEFAULT_HARNESS_TYPES: Dict[Benchmark, HarnessType] = {
    Benchmark.BEVFormer: HarnessType.Custom,
}
"""Dict[Benchmark, HarnessType]: Defines the default harnesses used for each benchmark."""
