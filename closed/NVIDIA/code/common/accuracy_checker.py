...
with ScopedRestrictedImport(_new_path):
    ...
    _version_str = VERSION if VERSION in _submission_model_config else "v0.5"
...
class BEVFormerAccuracyChecker(AccuracyChecker):
    def __init__(self, log_file: str, benchmark_conf: Dict[str, Any]):
        super().__init__(log_file,
                         benchmark_conf,
                         "bevformer",
                         "automotive/camera-3d-detection/accuracy_nuscenes_cpu.py")
        self.nuscenes_dir = Path(os.getenv("MLPERF_DATA_DIR", "/work/build/data")) / "nuScenes"

    def get_cmd(self) -> _AccuracyScriptCommand:
        argv = [os.path.join(G_MLCOMMONS_REPO_PATH, self.mlcommons_module_path),
                f"--mlperf-accuracy-file={self.log_file}",
                f"--nuscenes-dir={self.nuscenes_dir}",
                ]
        return _AccuracyScriptCommand(Benchmark.PYTHON_PATH(self.benchmark), argv, dict())
    

G_ACCURACY_CHECKER_MAP = {Benchmark.BEVFormer: BEVFormerAccuracyChecker,}
