...
def cpu_clock_str(cpu_cores: int, cpu_freq: int) -> str:
    cpu_clock_min = 115200
    cpu_clock_str = ''
    if (cpu_cores) > 0:
        cpu_clock_str += f"""
CPU_V3AE_0 MIN_FREQ {cpu_clock_min}
CPU_V3AE_0 MAX_FREQ {cpu_freq}
        """
    if (cpu_cores) > 4:
        cpu_clock_str += f"""
CPU_V3AE_1 MIN_FREQ {cpu_clock_min}
CPU_V3AE_1 MAX_FREQ {cpu_freq}
        """
    if (cpu_cores) > 8:
        cpu_clock_str += f"""
CPU_V3AE_2 MIN_FREQ {cpu_clock_min}
CPU_V3AE_2 MAX_FREQ {cpu_freq}
        """
    return cpu_clock_str


# FIXME: jsuh: this needs to be updated to support ThorX
# FIXME: WY: I added MAXN mode as -m 0, this seems to preventing strange
# bug in L4T that if any call fails, it will keep the nvpmodel state (over /etc/nvpmodel.conf)
# when calling without -f, take this out when l4t fix the bug.
nvpmodel_template_thorx = """
...
CORE_12 /sys/devices/system/cpu/cpu12/online
CORE_13 /sys/devices/system/cpu/cpu13/online
...
< PARAM TYPE=CLOCK NAME=CPU_V3AE_0 >
...
< PARAM TYPE=CLOCK NAME=CPU_V3AE_1 >
...
< PARAM TYPE=CLOCK NAME=CPU_V3AE_2 >
...
# MAXQ is the power model to maximize perf per watt
...
CPU_ONLINE CORE_12 {cpu_core[12]}
CPU_ONLINE CORE_13 {cpu_core[13]}
...
PVA0_VPS MAX_FREQ {pva_clock}
PVA0_AXI MAX_FREQ {pva_axi_clock}
