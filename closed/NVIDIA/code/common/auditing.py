...
from code.common.constants import G_MLCOMMONS_REPO_PATH, G_RESULTS_STAGING_PATH, AuditTest
...
class AuditTest01Verifier(AuditVerifier):
    def __init__(self, harness):
        super().__init__(AuditTest.TEST01, os.path.join(G_MLCOMMONS_REPO_PATH, "compliance", "TEST01",
                                                        "run_verification.py"))
...
class AuditTest04Verifier(AuditVerifier):
    def __init__(self, harness):
        super().__init__(AuditTest.TEST04, os.path.join(G_MLCOMMONS_REPO_PATH, "compliance", "TEST04",
                                                        "run_verification.py"))
        self.harness = harness
...
class AuditTest06Verifier(AuditVerifier):
    def __init__(self, harness):
        super().__init__(AuditTest.TEST06, os.path.join(G_MLCOMMONS_REPO_PATH, "compliance", "TEST06",
                                                        "run_verification.py"))
        self.harness = harness
...
def load(audit_test, benchmark):
    ...
    src_config = os.path.join(G_MLCOMMONS_REPO_PATH, 'compliance', audit_test, benchmark_name, 'audit.config')
    ...
    if not os.path.isfile(src_config):
        ...
        src_config = os.path.join(G_MLCOMMONS_REPO_PATH, 'compliance', audit_test, 'audit.config')
