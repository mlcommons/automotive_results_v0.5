...
from code.common.nvpmodel_soc import nvpmodel_template_thorx, cpu_clock_str
...
class SoCPowerStateController(PowerStateController):
    ...
    def power_state_target(self):
        ...
        num_cores = self.benchmark_conf["soc_num_cores"] = extract_field(self.main_args, self.benchmark_conf, "soc_num_cores")
        _ = self.benchmark_conf["soc_skip_maxq_reset"] = extract_field(self.main_args, self.benchmark_conf, "soc_skip_maxq_reset")

        # if any of these flags are set, all of them should be
        frequencies = [gpu_freq, cpu_freq, emc_freq, pva_freq, num_cores]
    ...
    def set_power_state(self, target: PowerState) -> PowerState:
        logging.info(f"Setting power state on SoC")

        num_cores_total = 0
        assert (self.benchmark_conf["system_id"] in {"ThorX"}), f"Unrecognized System {self.benchmark_conf['system_id']}!"
        if self.benchmark_conf["system_id"] == "ThorX":
            num_cores_total = 14
        else:
            num_cores_total = 8
        ...
        with open("build/nvpmodel.temp.conf", "w") as f:
            if self.benchmark_conf["system_id"] in {"ThorX"}:
                # FIXME: jsuh: need to update nvpmodel template for ThorX
                assert False, "Need to update nvpmodel template for ThorX"
                f.write(nvpmodel_template_thorx.format(gpu_clock=target.gpu_freq,
                ...
                logging.info(f"Skipping nvpmodel configuration for unknown system: {self.benchmark_conf['system_id']}")
                return None
...
def get_power_controller(main_args: Dict[str, Any], benchmark_conf: Dict[str, Any]):
    ...
    elif "is_soc" in DETECTED_SYSTEM.extras["tags"]:
        return SoCPowerStateController(main_args, benchmark_conf)
