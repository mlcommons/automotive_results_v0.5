...
def find_result_candidates(log_dir: str) -> List[str]:
    """Searches in `log_dir` for subdirectories containing required Loadgen logs for a valid MLPerf Automotive run:
    ...
    Returns:
        A list of all subdirectories of `log_dir` which contain all required files for MLPerf Automotive run
    """
...
def is_power_run(logdir: str, md: Optional[Dict[str, Any]] = None) -> Tuple[bool, str]:
    ...
    Args:
        logdir: The MLPerf Automotive run log directory to check
        md (Optional[Dict[str, Any]]): Pre-loaded metadata for an MLPerf Automotive harness run. If not set, reads from
...
def contains_valid_mlpauto_run(logdir: str, md: Optional[Dict[str, Any]] = None) -> bool:
...
def build_candidate_tree(candidate_list: List[str]) -> Tree:
    """Helper method to parse a list of directories containing valid MLPerf Automotive runs into an easily indexable
    structure. This method also discards any directories which contain logs for an INVALID MLPerf Automotive run (for
    clarification, INVALID here is in the context of the validity / submittability of an MLPerf Automotive result).

    Args:
        candidate_list (List[str]): A list of directories which contain all required MLPerf Automotive run files
    ...
    for c in candidate_list:
        ...
        if not contains_valid_mlpauto_run(c, md=md):
            continue
...
def find_best_candidate(candidate_list: List[str], result_dir: Optional[str] = None, regression_threshold: float = 0.98, force_update: bool = False, fail_fast: Optional[bool] = False):
    """From a list of directories containing MLPerf Automotive logfiles, finds the logs with the best performance results
    for submission. Performance runs will not be considered if there is no corresponding Accuracy run (and vice versa)

    Args:
        candidate_list (List[str]): A list of directories which contain all required MLPerf Automotive run files
        result_dir (Optional[str]): Path to root directory of MLPerf Automotive submission results to compare against. If
        ...
        regression_threshold (float): Float between 0.0 and 1.0 (inclusive) representing the minimum allowed
                                                percentage of an existing MLPerf Automotive result that can be
    ...
    for system_name, benchmarks in candidates.tree.items():
        ...
        for benchmark, scenarios in benchmarks.items():
            ...
            for scenario, runs in scenarios.items():
                ...
                logger.inc_indent_level()
                if benchmark in ["ssd-resnet50", "deeplabv3plus"]:
...
def get_sorted_perf_vals(log_dirs: list, system_id: str, benchmark: str, scenario: str) -> list:
    ...
    sorting_criteria_by_metric = {
        "result_99.90_percentile_latency_ns": SortingCriteria.Lower,
        "result_99.90_percentile_per_query_latency_ns": SortingCriteria.Lower,
